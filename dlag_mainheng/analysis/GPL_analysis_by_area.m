%% =========================================================================
% GPL_analysis_by_area
%
% Purpose:
% Analyze a grating/plaid direction-tuning run after first restricting the
% analysis to explicitly mapped model groups in a separate target run.
%
% Main steps:
% 1. The user specifies one probe ID and one area name for every model group.
%    Array position is the model-group index used by DLAG/reconstruction.
% 2. Validate that this mapping exactly matches target_run.group_probe and
%    target_run.group_name in model_data_allruns.mat.
% 3. Read the selected unit IDs and per-unit area names for each probe, then
%    keep only the units belonging to the mapped area of each model group.
% 4. Load each probe/kilosort GPL run only once and analyze every requested
%    model group belonging to that probe.
% 5. Keep only the group-selected target units that also exist in the GPL run.
% 6. Average raw_fr across bins to obtain one mean firing rate per trial.
% 7. Compute grating and plaid direction tuning curves (mean, SD, SEM).
% 8. Optionally load a matching spontaneous/baseline data file.
% 9. Compute grating DI and vector-based OSI, with optional baseline subtraction.
% 10. Build pattern and component predictions for the plaid tuning curve.
% 11. Compute ordinary correlations, partial correlations, Fisher Z values,
%    pattern index PI = Zp - Zc, and pattern/component classification.
% 12. Save numerical results and all figures as both PNG and FIG files.
%
...
% - unit_gpl_results_sponsub.mat, when subtract_spontaneous = true
% - Group1_V1_Grating_tuning_SEM-or-SD.png and .fig
% - Group1_V1_Plaid_tuning_SEM-or-SD.png and .fig
% - Group1_V1_DI_OSI_distribution.png and .fig
% - Group1_V1_Pattern_component_analysis.png and .fig
%
% One MAT is saved per probe/kilosort folder. All mapped model groups on that
% probe are stored in gpl_results.group. Use gpl_results.group_indices and
% each entry's model_group_index to preserve the original model-group order.
%
% Notes:
...
% 6. Area names do not need to be globally unique. For example,
%       group_probes = [0, 0, 1]
%       group_names  = {'V1', 'V2', 'V1'}
%    is valid because model-group index and probe ID preserve the mapping.
% 7. The same probe/area pair may not appear twice because per-unit area
%    labels alone cannot distinguish two such populations.
% 8. Figure filenames contain both model-group index and area name. The MAT
%    filename remains unchanged and contains all groups for that probe.
% =========================================================================
...
% Explicit model-group mapping. Array position is the model-group index.
% The following example means:
%   model group 1 -> probe 0, V1
%   model group 2 -> probe 0, V2
%   model group 3 -> probe 1, V1
% This mapping is checked against target_run.group_probe and group_name.
group_probes = [0, 1];
group_names = {'V1', 'MT'};
...
% Options: 'SEM' or 'SD'. Both are saved for each group inside the MAT.
error_method = 'SD';
...
% Base output MAT filename. Only the optional _sponsub suffix is inserted.
% The same filename is used in different probe/kilosort folders:
%   unit_gpl_results.mat
...
group_names = normalize_text_list_local(group_names, 'group_names');
group_names = reshape(group_names, 1, []);
group_probes = double(group_probes(:)');

if isempty(group_names)
    error('group_names must contain at least one area name.');
end

if numel(group_probes) ~= numel(group_names)
    error('group_probes and group_names must have the same length.');
end
...
for g = 1:numel(group_names)
    earlier_same_pair = group_probes(1:g-1) == group_probes(g) & ...
        strcmp(group_names(1:g-1), group_names{g});
    if any(earlier_same_pair)
        error(['The same probe/area pair appears more than once: probe %d, ', ...
            'area %s. Per-unit area labels cannot distinguish these groups.'], ...
            group_probes(g), group_names{g});
    end
end
...
group_selections = resolve_target_group_selections_local( ...
    target_run, group_probes, group_names);

%% ----------------------- Process each requested probe -----------------------
probes_to_process = unique(group_probes, 'stable');
...
for probe_list_idx = 1:numel(probes_to_process)

    thisProbe = probes_to_process(probe_list_idx);
    this_group_indices = find(group_probes == thisProbe);
    imecStr = sprintf('imec%d', thisProbe);
    probe_folder = fullfile(cat_folder, [run_g '_' imecStr]);

    fprintf('\n============================================================\n');
    fprintf('Processing probe %d, model groups %s\n', ...
        thisProbe, mat2str(this_group_indices));
    ...
    for local_group_idx = 1:numel(this_group_indices)
        model_group_index = this_group_indices(local_group_idx);
        selection = group_selections(model_group_index);
        fprintf(['  Group %d -> %s | unit-ID field %s | ', ...
            'unit-area field %s | units %d\n'], ...
            model_group_index, selection.group_name, ...
            selection.target_unit_id_field, ...
            selection.target_unit_groupname_field, ...
            numel(selection.target_unit_ids));
    end
    ...
    for i = 1:numel(d)
        ...
        try
            ...
            %% ----------------------- Compute every group on this probe -----------------------
            ...
            for local_group_idx = 1:numel(this_group_indices)
                ...
                fprintf('  Computing Group %d -> probe %d -> %s\n', ...
                    model_group_index, thisProbe, selection.group_name);

                this_result = compute_gpl_results( ...
                    gpl_data, ...
                    selection.target_unit_ids, ...
                    selection.target_unit_id_field, ...
                    selection.target_unit_groupname_field, ...
                    selection.target_groupd_field, ...
                    model_group_index, ...
                    selection.target_expected_unit_count, ...
                    thisProbe, ...
                    selection.group_name, ...
                    selection.area_file_tag, ...
                    selection.group_file_tag, ...
                    ksDir, ...
                    dat_file, ...
                    target_run_idx, ...
                    gpl_run_idx, ...
                ...
                base_grating = sprintf('%s_Grating_tuning_%s', ...
                    selection.group_file_tag, error_method);
                base_plaid = sprintf('%s_Plaid_tuning_%s', ...
                    selection.group_file_tag, error_method);

                % Selectivity and pattern/component results change with
                % baseline subtraction, so these names receive _sponsub.
                base_selectivity = sprintf('%s_DI_OSI_distribution%s', ...
                    selection.group_file_tag, output_suffix);
                base_pattern = sprintf( ...
                    '%s_Pattern_component_analysis%s', ...
                    selection.group_file_tag, output_suffix);

                this_result.output_suffix = output_suffix;
                this_result.files = struct();
                this_result.files.mat = fullfile(ksDir, output_mat_name);
                this_result.files.grating_png = fullfile(ksDir, ...
                    [base_grating '.png']);
                this_result.files.grating_fig = fullfile(ksDir, ...
                    [base_grating '.fig']);
                this_result.files.plaid_png = fullfile(ksDir, ...
                    [base_plaid '.png']);
                this_result.files.plaid_fig = fullfile(ksDir, ...
                    [base_plaid '.fig']);
                this_result.files.selectivity_png = fullfile(ksDir, ...
                    [base_selectivity '.png']);
                this_result.files.selectivity_fig = fullfile(ksDir, ...
                    [base_selectivity '.fig']);
                this_result.files.pattern_component_png = fullfile(ksDir, ...
                    [base_pattern '.png']);
                this_result.files.pattern_component_fig = fullfile(ksDir, ...
                    [base_pattern '.fig']);

                if isempty(group_results)
                    group_results = this_result;
                else
                    group_results(end + 1) = this_result; %#ok<SAGROW>
                end
            end

            %% ----------------------- Build one probe-level MAT structure -----------------------
            gpl_results = struct();
            gpl_results.format_version = 'grouped_by_probe_v1';
            gpl_results.probe_id = thisProbe;
            ...
            gpl_results.baseline_subtracted = ...
                logical(subtract_spontaneous);
            gpl_results.output_suffix = output_suffix;
            gpl_results.group_indices = this_group_indices(:)';
            gpl_results.group_probes = ...
                group_probes(this_group_indices);
            gpl_results.group_names = group_names(this_group_indices);
            gpl_results.model_group_indices = 1:numel(group_names);
            gpl_results.model_group_probes = group_probes;
            gpl_results.model_group_names = group_names;
            gpl_results.num_groups_in_file = numel(group_results);
            gpl_results.num_model_groups = numel(group_names);
            gpl_results.group = group_results;
            gpl_results.analysis_window = group_results(1).analysis_window;
            gpl_results.bin_size = group_results(1).bin_size;
            gpl_results.pattern_z_threshold = pattern_z_threshold;
            gpl_results.plaid_component_separation_deg = ...
                group_results(1).plaid_component_separation_deg;
            ...
            % Save once before plotting so all numerical results remain
            % available even if a later figure export fails.
            save(gpl_results.files.mat, 'gpl_results');

            %% ----------------------- Plot every group on this probe -----------------------
            for local_group_idx = 1:numel(group_results)
                this_result = group_results(local_group_idx);
                model_group_index = this_result.model_group_index;
                group_name = this_result.group_name;
                group_display_name = sprintf( ...
                    'Group %d: %s', model_group_index, group_name);

                fig = plot_direction_tuning( ...
                    this_result.grating.directions, ...
                    this_result.grating.mean_fr, ...
                    select_error_matrix(this_result.grating, error_method), ...
                    this_result.plot.depth_desc_order, ...
                    sprintf('%s grating tuning (%s)', ...
                    group_display_name, error_method), ...
                    figure_visibility);
                save_figure_pair(fig, ...
                    this_result.files.grating_png, ...
                    this_result.files.grating_fig);
                close(fig);

                fig = plot_direction_tuning( ...
                    this_result.plaid.directions, ...
                    this_result.plaid.mean_fr, ...
                    select_error_matrix(this_result.plaid, error_method), ...
                    this_result.plot.depth_desc_order, ...
                    sprintf('%s plaid tuning (%s)', ...
                    group_display_name, error_method), ...
                    figure_visibility);
                save_figure_pair(fig, ...
                    this_result.files.plaid_png, ...
                    this_result.files.plaid_fig);
                close(fig);

                fig = plot_di_osi_distributions( ...
                    this_result.grating.DI, ...
                    this_result.grating.OSI, ...
                    ...
                    subtract_spontaneous);
                save_figure_pair(fig, ...
                    this_result.files.selectivity_png, ...
                    this_result.files.selectivity_fig);
                close(fig);

                fig = plot_pattern_component_analysis( ...
                    this_result.plaid.Zp, ...
                    this_result.plaid.Zc, ...
                    this_result.plaid.PI, ...
                    this_result.plaid.is_pattern, ...
                    this_result.plaid.is_component, ...
                    this_result.plaid.is_unclassified, ...
                    pattern_z_threshold, ...
                    distribution_bar_width, ...
                    group_display_name, ...
                    figure_visibility);
                save_figure_pair(fig, ...
                    this_result.files.pattern_component_png, ...
                    this_result.files.pattern_component_fig);
                close(fig);

                fprintf('  Saved Group %d (%s) figures.\n', ...
                    model_group_index, group_name);
            end

            % Save again after all expected output paths have been recorded.
            save(gpl_results.files.mat, 'gpl_results');

            fprintf('Saved probe-level GPL MAT:\n  %s\n', ...
                gpl_results.files.mat);

        catch ME
            fprintf(2, 'Error for probe %d, ksDir %s\n', ...
                thisProbe, ksDir);
...
function selections = resolve_target_group_selections_local( ...
        target_run, requested_group_probes, requested_group_names)
% Validate the explicit group/probe/area mapping, then select each model
% group's target-run unit IDs. Model-group index is the array position.

% Group-level mapping:
%   target_run.group_name
%   target_run.group_probe
%
% Per-unit mapping, normal strategies:
%   probeN_usedunit_ids
%   probeN_usedunit_groupname
%
% Per-unit mapping, nan_trial_strategy == 6:
%   raw_fr_probeN_usedunit_ids
%   raw_fr_probeN_usedunit_groupname

    required_fields = {'group_name', 'group_probe'};
    for k = 1:numel(required_fields)
        if ~isfield(target_run, required_fields{k})
            error('Target run is missing field %s.', required_fields{k});
        end
    end

    model_group_names = normalize_text_list_local( ...
        target_run.group_name, 'target_run.group_name');
    model_group_names = reshape(model_group_names, 1, []);
    model_group_probes = double(target_run.group_probe(:)');

    if isempty(model_group_names) || ...
            numel(model_group_names) ~= numel(model_group_probes)
        error(['target_run.group_name and target_run.group_probe must have ', ...
            'the same nonzero length.']);
    end

    if any(~isfinite(model_group_probes)) || ...
            any(model_group_probes < 0) || ...
            any(model_group_probes ~= round(model_group_probes))
        error('target_run.group_probe must contain finite 0-based integers.');
    end

    requested_group_names = normalize_text_list_local( ...
        requested_group_names, 'requested_group_names');
    requested_group_names = reshape(requested_group_names, 1, []);
    requested_group_probes = double(requested_group_probes(:)');

    if numel(requested_group_names) ~= numel(model_group_names) || ...
            numel(requested_group_probes) ~= numel(model_group_probes)
        error(['The explicit mapping contains %d groups, but target_run ', ...
            'contains %d model groups. group_probes and group_names must ', ...
            'describe every model group in its original order.'], ...
            numel(requested_group_names), numel(model_group_names));
    end

    for g = 1:numel(model_group_names)
        if requested_group_probes(g) ~= model_group_probes(g)
            error(['Mapping mismatch for model group %d: group_probes(%d) ', ...
                'is %d, but target_run.group_probe(%d) is %d.'], ...
                g, g, requested_group_probes(g), g, model_group_probes(g));
        end
        if ~strcmp(requested_group_names{g}, model_group_names{g})
            error(['Mapping mismatch for model group %d: group_names{%d} ', ...
                'is %s, but target_run.group_name{%d} is %s.'], ...
                g, g, requested_group_names{g}, g, model_group_names{g});
        end
    end

    empty_selection = struct( ...
        'group_name', '', ...
        'area_name', '', ...
        'area_file_tag', '', ...
        'group_file_tag', '', ...
        ...
        'probe_id', [], ...
        'target_unit_ids', [], ...
        'target_unit_id_field', '', ...
        'target_unit_groupname_field', '', ...
        'target_groupd_field', '', ...
        'target_expected_unit_count', []);

    selections = repmat(empty_selection, 1, numel(model_group_names));

    for model_group_index = 1:numel(model_group_names)
        area_name = requested_group_names{model_group_index};
        probe_id = requested_group_probes(model_group_index);

        same_probe_area = model_group_probes == probe_id & ...
            strcmp(model_group_names, area_name);
        if sum(same_probe_area) ~= 1
            error(['Probe %d and area %s identify %d model groups. This ', ...
                'program requires each probe/area pair to identify exactly ', ...
                'one model group.'], ...
                probe_id, area_name, sum(same_probe_area));
        end

        [probe_unit_ids, probe_unit_area_names, id_field, ...
            groupname_field, use_rawfr_metadata] = ...
            get_target_probe_unit_metadata_local(target_run, probe_id);

        [target_groupd, target_groupd_field] = ...
            get_target_groupd_local(target_run, use_rawfr_metadata);

        if numel(target_groupd) ~= numel(model_group_names)
            error(['%s has %d entries, but target_run.group_name has %d ', ...
                'entries.'], target_groupd_field, numel(target_groupd), ...
                numel(model_group_names));
        end

        expected_unit_count = target_groupd(model_group_index);
        take_area = strcmp(probe_unit_area_names, area_name);
        selected_unit_ids = probe_unit_ids(take_area);

        if isempty(selected_unit_ids)
            error(['No target-selected units for area %s were found in %s ', ...
                'for probe %d.'], area_name, groupname_field, probe_id);
        end

        if numel(selected_unit_ids) ~= expected_unit_count
            error(['Area-unit count mismatch for area %s, model group %d, ', ...
                'probe %d: %s reports %d units, but filtering %s by area ', ...
                'name found %d unit IDs.'], ...
                area_name, model_group_index, probe_id, ...
                target_groupd_field, expected_unit_count, ...
                groupname_field, numel(selected_unit_ids));
        end

        if any(~isfinite(selected_unit_ids))
            error('Target-selected unit IDs for area %s contain nonfinite values.', ...
                area_name);
        end

        if numel(unique(selected_unit_ids)) ~= numel(selected_unit_ids)
            error('Target-selected unit IDs for area %s contain duplicates.', ...
                area_name);
        end

        area_file_tag = make_filename_safe_local(area_name);

        group_file_tag = sprintf('Group%d_%s', ...
            model_group_index, area_file_tag);

        selections(model_group_index).group_name = area_name;
        selections(model_group_index).area_name = area_name;
        selections(model_group_index).area_file_tag = area_file_tag;
        selections(model_group_index).group_file_tag = group_file_tag;
        selections(model_group_index).model_group_index = model_group_index;
        selections(model_group_index).probe_id = probe_id;
        selections(model_group_index).target_unit_ids = selected_unit_ids(:);
        selections(model_group_index).target_unit_id_field = id_field;
        selections(model_group_index).target_unit_groupname_field = ...
            groupname_field;
        selections(model_group_index).target_groupd_field = ...
            target_groupd_field;
        selections(model_group_index).target_expected_unit_count = ...
            expected_unit_count;
    end
end

function [unit_ids, unit_area_names, id_field, groupname_field, ...
        use_rawfr_metadata] = ...
        get_target_probe_unit_metadata_local(target_run, probe_id)
% Read matched unit-ID and per-unit area-name fields for one probe.

    standard_id_field = sprintf('probe%d_usedunit_ids', probe_id);
    standard_groupname_field = ...
        sprintf('probe%d_usedunit_groupname', probe_id);
    rawfr_id_field = sprintf('raw_fr_probe%d_usedunit_ids', probe_id);
    rawfr_groupname_field = ...
        sprintf('raw_fr_probe%d_usedunit_groupname', probe_id);

    has_standard_id = isfield(target_run, standard_id_field);
    has_standard_groupname = isfield(target_run, standard_groupname_field);
    has_rawfr_id = isfield(target_run, rawfr_id_field);
    has_rawfr_groupname = isfield(target_run, rawfr_groupname_field);

    if xor(has_standard_id, has_standard_groupname)
        error(['Target run contains only one of the paired fields %s and %s.'], ...
            standard_id_field, standard_groupname_field);
    end

    if xor(has_rawfr_id, has_rawfr_groupname)
        error(['Target run contains only one of the paired fields %s and %s.'], ...
            rawfr_id_field, rawfr_groupname_field);
    end

    strategy_six = isfield(target_run, 'nan_trial_strategy') && ...
        isequal(double(target_run.nan_trial_strategy), 6);

    if strategy_six
        if ~(has_rawfr_id && has_rawfr_groupname)
            error(['nan_trial_strategy == 6 requires %s and %s for this ', ...
                'raw-fr GPL analysis.'], rawfr_id_field, rawfr_groupname_field);
        end
        id_field = rawfr_id_field;
        groupname_field = rawfr_groupname_field;
        use_rawfr_metadata = true;
    elseif has_standard_id && has_standard_groupname
        id_field = standard_id_field;
        groupname_field = standard_groupname_field;
        use_rawfr_metadata = false;
    elseif has_rawfr_id && has_rawfr_groupname
        id_field = rawfr_id_field;
        groupname_field = rawfr_groupname_field;
        use_rawfr_metadata = true;
    else
        error(['Target run contains neither a complete standard nor a ', ...
            'complete raw_fr unit-metadata pair for probe %d.'], probe_id);
    end

    unit_ids = double(target_run.(id_field));
    unit_ids = unit_ids(:);
    unit_area_names = normalize_text_list_local( ...
        target_run.(groupname_field), ...
        ['target_run.' groupname_field]);
    unit_area_names = unit_area_names(:);

    if isempty(unit_ids)
        error('Target-selected unit list is empty in field %s.', id_field);
    end

    if numel(unit_area_names) ~= numel(unit_ids)
        error(['%s contains %d IDs, but %s contains %d area names.'], ...
            id_field, numel(unit_ids), groupname_field, ...
            numel(unit_area_names));
    end
    ...
    if numel(unique(unit_ids)) ~= numel(unit_ids)
        error('Target-selected unit IDs in %s contain duplicates.', id_field);
    end
end

function [groupd, field_name] = ...
        get_target_groupd_local(target_run, use_rawfr_metadata)
% Read the group sizes corresponding to the selected unit-metadata version.

    if use_rawfr_metadata
        field_name = 'raw_fr_groupd';
    else
        field_name = 'groupd';
    end

    if ~isfield(target_run, field_name)
        error('Target run is missing field %s.', field_name);
    end

    groupd = double(target_run.(field_name));
    groupd = groupd(:)';

    if isempty(groupd) || any(~isfinite(groupd)) || ...
            any(groupd <= 0) || any(groupd ~= round(groupd))
        error('%s must contain positive finite integer group sizes.', ...
            field_name);
    end
end

function names = normalize_text_list_local(raw_names, parameter_name)
% Normalize text input to a cell array of nonempty character vectors.

    if isstring(raw_names)
        names = cellstr(raw_names(:));
    elseif ischar(raw_names)
        if size(raw_names, 1) == 1
            names = {raw_names};
        else
            names = cellstr(raw_names);
        end
    elseif iscategorical(raw_names)
        names = cellstr(string(raw_names(:)));
    elseif iscell(raw_names)
        names = raw_names(:);
    else
        error('%s must be text or a cell array of text.', parameter_name);
    end
    ...
    for k = 1:numel(names)
        ...
        if ~ischar(value)
            error('%s{%d} must be a character vector or string scalar.', ...
                parameter_name, k);
        end
        value = strtrim(value);
        if isempty(value)
            error('%s{%d} cannot be empty.', parameter_name, k);
        end
        names{k} = value;
    end
end

function safe_name = make_filename_safe_local(value)
% Convert one area name to a portable filename component.
    ...
    if isempty(safe_name)
        error('Area name %s does not contain a usable filename character.', ...
            value);
    end
end
...
function gpl_results = compute_gpl_results( ...
        ...
        target_unit_groupname_field, target_groupd_field, ...
        model_group_index, target_expected_unit_count, ...
        probe_id, group_name, area_file_tag, group_file_tag, ...
        ksDir, dat_file, ...
...
% Compute tuning, selectivity, and pattern/component metrics for one area.
    ...
    if isempty(used_unit_ids)
        error(['No target-selected units for area %s, probe %d were found ', ...
            'in the GPL run of %s.'], group_name, probe_id, ksDir);
    end

    if ~isempty(missing_target_unit_ids)
        warning(['%d/%d target-selected units were not found in the GPL run ' ...
            'for area %s, probe %d. Example missing unit ID: %g'], ...
            numel(missing_target_unit_ids), numel(target_unit_ids), ...
            group_name, probe_id, missing_target_unit_ids(1));
    end
    ...
    % group_name is retained for compatibility with existing downstream
    % programs. In this area-based version it is exactly the area name.
    gpl_results.group_name = group_name;
    gpl_results.area_name = group_name;
    gpl_results.area_file_tag = area_file_tag;
    gpl_results.group_file_tag = group_file_tag;
    gpl_results.model_group_index = model_group_index;
    gpl_results.ksDir = ksDir;
    ...
    gpl_results.target_unit_groupname_field = ...
        target_unit_groupname_field;
    gpl_results.target_groupd_field = target_groupd_field;
    gpl_results.target_expected_unit_count = target_expected_unit_count;
