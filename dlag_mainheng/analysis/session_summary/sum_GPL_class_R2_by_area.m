...
% several model groups. For every session this script scans all such files
% and identifies each nested gpl_results.group entry strictly from its
% model_group_index. No user-supplied probe mapping is required.
...
% allowed because every group-specific output tag contains the group index.
group_names = {'V1', 'MT'};
...
output_prefix = sprintf('%s_%s_GPLclass_R2_%s', ...
...
for si = 1:numel(session_dirs)
    ...
    try
        [rec, reference] = readOneSessionLocal( ...
            ...
            group_file_tags, gpl_mat_name, field_names, stim_types, ...
            reference);
...
for g = 1:numel(group_names)
    for st = 1:numel(stim_types)
        ...
        fig = plotOneSummaryLocal( ...
            records, g, stim_name, group_display_names{g}, ...
...
function [rec, reference] = readOneSessionLocal( ...
    session_dir, data_content, model_mode, runIdx, ...
    reconstruction_suffix, group_names, group_display_names, ...
    group_file_tags, gpl_mat_name, field_names, stim_types, reference)
    ...
    validateStimtypeR2Local( ...
        R, data_content, model_mode, num_groups, field_names, ...
        stim_types, r2_file);

    reference = validateOrInitializeReferenceLocal( ...
        reference, R, group_names, group_display_names, ...
        group_file_tags, field_names, stim_types, session_dir);
...
function reference = validateOrInitializeReferenceLocal( ...
    ...
    field_names, stim_types, session_dir)
...
function M = buildMultiSessionSummaryLocal( ...
    ...
    class_names, class_labels, field_names, field_labels, stim_types, ...
    output_prefix, output_base, group_colors_base, ...
    session_colors_by_group, light_start, light_end)
    ...
    % M.labels.group is retained for compatibility with code that already
    % reads this field. It now contains the complete display label.
    ...
    M.notes.group_name_usage = [ ...
        'Manual group_names are display/file labels only and are not ', ...
        'compared with stored group or area names.'];
...
function fig = plotOneSummaryLocal( ...
    records, group_idx, stim_name, group_display_name, ...
    class_names, class_labels, field_labels, ...
