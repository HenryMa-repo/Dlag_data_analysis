...
%
% With two model groups, by_stimtype produces four figures and
% by_three_features produces sixteen figures (eight per model group).
...
validateUserSettingsLocal( ...
    root_dir, data_content, model_mode, r2_split_mode, runIdx, ...
...
if strcmp(r2_split_mode, 'by_stimtype')
    ...
    output_prefix = sprintf( ...
        '%s_%s_stszct_GPLclass_deltaR2_zscore_acr_vs_wit_ff_vs_fb', ...
        data_content, model_tag);
end
...
for si = 1:numel(session_dirs)
    ...
    try
        [rec, reference] = readOneSessionLocal( ...
            ...
            delta_positive_fields, delta_negative_fields, ...
            stim_types, category_labels, reference);
...
for g = 1:numel(group_names)
    for st = 1:numel(stim_types)
        ...
        category_label = category_labels{st};

        fig = plotOneDeltaSummaryLocal( ...
            MultiSessionGPLClassDeltaR2.pooled(g), ...
            stim_name, category_label, group_display_names{g}, ...
            class_names, class_labels, class_markers, ...
        ...
        figure_base = sprintf('%s_%s_%s', ...
            output_prefix, group_file_tags{g}, category_label);
...
function M = buildMultiSessionDeltaSummaryLocal( ...
    ...
    M.meta.format_version = 'GPL_class_deltaR2_by_area_v2';
...
function fig = plotOneDeltaSummaryLocal( ...
    pooled_group, stim_name, category_label, group_display_name, ...
    class_names, class_labels, class_markers, ...
    ...
    figure_name = sprintf('%s %s GPL-class delta R2', ...
        group_display_name, categoryDisplayNameLocal(category_label));
